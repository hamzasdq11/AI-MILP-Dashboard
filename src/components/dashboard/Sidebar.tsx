...
export default function Sidebar({ activeSection, onSectionChange }: SidebarProps) {
  return (
    <motion.aside
      ...
      className="w-[260px] min-w-[260px] h-screen border-r border-border flex flex-col"
      style={{ background: "hsl(var(--sidebar-background))" }}
    >
      {/* Logo */}
      <div className="p-5 border-b border-border">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-md flex items-center justify-center" style={{ background: "hsl(var(--primary) / 0.15)" }}>
            <Brain className="w-4.5 h-4.5" style={{ color: "hsl(var(--primary))" }} />
          </div>
          <div>
            <h1 className="text-sm font-semibold text-foreground tracking-tight">MILP Allocator</h1>
            <p className="text-[10px] text-muted-foreground">IIM Ranchi · Term IV</p>
          </div>
        </div>
      </div>
